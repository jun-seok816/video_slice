...
type YtDlpUpdateMethod = "pipx" | "pip";
...
function toPublicDataPath(filePath: string) {
...
function getPythonBin() {
  return (
    process.env.YTDLP_PYTHON_BIN?.trim() ||
...
function getYtDlpUpdateMethod(): YtDlpUpdateMethod {
  const updateMethod = process.env.YTDLP_UPDATE_METHOD?.trim().toLowerCase();

  if (updateMethod === "pip") return "pip";
  return "pipx";
}
...
function mapYtDlpErrorMessage(stderr: string) {
...
function runYtDlpDownload(req: Request, args: string[]) {
  return new Promise<string[]>((resolve, reject) => {
    ...
    ytDlpProcess.on("error", (err) => {
      ...
      reject(
        Error(
          `${ytDlpCommand.command} 실행에 실패했습니다. yt-dlp 설치, PATH, 또는 YTDLP_BIN 설정을 확인해 주세요. pipx로 설치했다면 YTDLP_BIN=${getPipxYtDlpBin()} 값을 사용할 수 있습니다. (${err.message})`
        )
      );
    });
...
function validateYoutubeUrl(
  req: Request<{}, {}, YoutubeUrlRequestBody>,
  res: Response,
  next: NextFunction
) {
  ...
  } as YoutubeUploadState;

  next();
}

// 다운로드 파일을 저장할 data/jobs/{jobId} 폴더를 준비합니다.
function prepareDownloadFolder(
  req: Request,
  res: Response,
  next: NextFunction
) {
