...
class TimeCode {
    ...
    eTime;
    start;
    constructor(text, sTime, eTime, id) {
    ...
    static commaTodot(commaTime) {
        return commaTime.replace(",", ".");
    }
    static sm_time2Sec(p_time, p_startTime = 0) {
    ...
    static sm_sec2time(p_time) {
        let pad = function (num, size) {
            return ("000" + num).slice(-size);
        }, time = parseFloat(p_time).toFixed(3), hours = Math.floor(parseFloat(time) / 60 / 60), minutes = Math.floor(parseFloat(time) / 60) % 60, seconds = Math.floor(parseFloat(time) - minutes * 60), milliseconds = time.slice(-3);
        return (pad(hours, 2) +
            ":" +
            pad(minutes, 2) +
            ":" +
    ...
    static sm_formatDuration(videoElement) {
        ...
        const seconds = Math.floor((totalMilliseconds - hours * 3600000 - minutes * 60000) / 1000);
    ...
    static sm_getDuration(startTime, endTime) {
